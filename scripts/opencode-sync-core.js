...
async function chooseTarget(action, force) {
  ...
  const target = await select({
    message: `${c.tn.cyan}?${R} What do you want to sync?`,
...
function runConfigSync(action, force, target) {
  ...
  return runScript(SYNC_CONFIG_SCRIPT, args, action === "status" ? "Checking config status..." : `${action === "push" ? "Pushing" : "Pulling"} config...`);
}
...
function runSessionSync(action, force, target) {
  ...
  if (force) args.push("--force");
  if (target) args.push(`--target=${target}`, `--action=${action}`);
  return runScript(SYNC_SESSIONS_SCRIPT, args, action === "status" ? "Checking session status..." : `${action === "push" ? "Pushing" : "Pulling"} sessions...`);
}
...
function showHelp() {
  ...
  console.log(`    ${c.tn.cyan}opencode-push-force${c.reset}    Force push (overwrite remote on conflict)`);
  console.log(`    ${c.tn.cyan}opencode-pull-force${c.reset}    Force pull (overwrite local changes)`);
  console.log(`    ${c.tn.cyan}opencode-push status${c.reset}   Show sync status`);
  console.log();
  section("Notes");
  note("Session data must be synced outside OpenCode; the script checks for running processes.");
  console.log();
}
